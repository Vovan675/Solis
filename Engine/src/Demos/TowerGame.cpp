...
static const glm::vec4 COLOR_BONUS = {1.0f, 0.0f, 0.0f, 1.0f};
...
Block createBlock(Scene* scene, const glm::vec3& size, const glm::vec3& pos, const glm::vec4& color) {
    ...
    transform.setLocalScale(size * 0.5f * 0.01f);
...
void Layer::tick(float dt, float colorTime) {
    if (fallingBlock) {
        ...
        fallingBlock->entity.getTransform().setPosition(fallingBlock->position);
    }
    if (movingBlock && !isBonusBlock) {
...
void Layer::spawnParticles() {
    ...
    for (int i = 0; i < count; ++i) {
        ...
        transform.setPosition(transform.getLocalPosition() + glm::vec3(0, 0.5, 0)); // Start slightly above
    }
}
...
void Tower::tick(float dt, float colorTime) {
    ...
    layers.back()->movingBlock->entity.getTransform().setPosition(pos);
}
...
void Tower::reset() {
    direction = 1;
    speedFactor = INITIAL_SPEED_FACTOR;
...
void TowerGame::initResources()
{
	...
	Entity entity = Model::createEntity(cube_model);
	auto &transform = entity.getTransform();
	transform.setLocalScale(glm::vec3(0.01, 0.001, 0.01));
	transform.setPosition(glm::vec3(0.0, -1.01, 0.0));
