...
void DX12Shader::recompile()
{
	...
	DynamicRHI::CompileShaderResult result = gDynamicRHI->compile_shader(path.wstring().c_str(), type, entry_point, false, &defines_ptrs);
	blob = result.data;
...
eastl::vector<CD3DX12_ROOT_PARAMETER1> DX12Shader::getRootParameters(eastl::vector<DX12Shader *> shaders, BindingInfo &binding_info)
{
	...
	for (auto &shader : shaders)
	{
		...
		bool is_library = shader->reflection_library != nullptr;

		if (is_library) 
		{
			D3D12_LIBRARY_DESC library_desc;
			HRESULT res = shader->reflection_library->GetDesc(&library_desc);
			for (int f = 0; f < library_desc.FunctionCount; f++)
			{
				ID3D12FunctionReflection *function = shader->reflection_library->GetFunctionByIndex(f);
				D3D12_FUNCTION_DESC function_desc = {};
				function->GetDesc(&function_desc);

				for (int i = 0; i < function_desc.BoundResources; i++)
				{
					D3D12_SHADER_INPUT_BIND_DESC bind_desc;
					function->GetResourceBindingDesc(i, &bind_desc);
					...
					} else if (bind_desc.Type == D3D_SIT_RTACCELERATIONSTRUCTURE)
					{
						auto &bindings = srv_by_space[bind_desc.Space];
						if (!isDefined(bindings, bind_desc.BindPoint))
						{
