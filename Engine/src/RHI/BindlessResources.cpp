...
void DX12BindlessResources::setTexture(uint32_t index, RHITexture *texture)
{
	...
	// TODO: remove, always use bindless samplers and pass indices to samplers instead of samplers itself
	D3D12_CPU_DESCRIPTOR_HANDLE cpu_handle_samplers_heap(rhi->samplers_heap->getHandle(std::min(index, 512u)).getCpuHandle());

	// Copy from staging heap, to current frame's shader visible heap
	rhi->device->CopyDescriptorsSimple(1, cpu_handle_samplers_heap, native_texture->sampler_view.getCpuHandle(), D3D12_DESCRIPTOR_HEAP_TYPE_SAMPLER);
}
...
void DX12BindlessResources::set_invalid_texture(uint32_t index)
{
	...
	// Copy from staging heap, to current frame's shader visible heap
	rhi->device->CopyDescriptorsSimple(1, cpu_handle_samplers_heap, texture->sampler_view.getCpuHandle(), D3D12_DESCRIPTOR_HEAP_TYPE_SAMPLER);
}
